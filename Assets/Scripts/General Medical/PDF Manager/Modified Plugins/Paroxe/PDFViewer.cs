...
#pragma warning restore 414
        ...
        private bool m_NowHorizontal;
        private bool m_ZoomOutMaxed = false;
        ...
        [SerializeField]
        private float m_MaxZoomFactor = 8.0f; //8.0f == 800%
        [SerializeField]
        private float m_MaxZoomFactorTextureQuality = 4.0f;
        [SerializeField]
        private float m_MinZoomFactor = 0.25f; //0.25f == 50%
        [SerializeField]
        ...
        [SerializeField]
        private bool m_ShowBookmarksViewer = false;
        [SerializeField]
        ...
        {
            if (m_Internal == null) m_Internal = FindObjectOfType<PDFViewerInternal>(false);
        }
        ...
        public bool NowHorizontal
        {
            get { return m_NowHorizontal; }
            set { m_NowHorizontal = value; }
        }
        ...
        public Vector2 ReferenceNormalPage
        {
            get { return m_NormalPageSizes[0];}
        }
        ...
        public float ZoomFactor
        {
            ...
            set
            {
                if (Math.Abs(m_ZoomToGo - Mathf.Clamp(value, MinZoomFactor, MaxZoomFactor)) > float.Epsilon)
                {
                    ...
                    if (!m_NowHorizontal) m_ZoomPosition = new Vector2(0.0f, m_Internal.Viewport.rect.size.y * 0.5f);
                    else m_ZoomPosition = new Vector2(m_Internal.Viewport.rect.size.y * 0.5f, 0.0f);
        ...
        {
            Rect viewportrect = m_Internal.Viewport.rect;
            switch (pageFitting)
            { 
                ...
                case PageFittingType.ViewerHeight:
                    {
                        ...
                        m_ZoomToGo = m_NowHorizontal ? viewportHeight / firstPageWidth 
                            : viewportHeight / firstPageHeight;
        ...
        {
            string[] tempsplit = new string[] { }, tempdot = new string[] { }, tempbslash = new string[] { },
                tempslash = new string[] { };
            string tempfilename = string.Empty;
            switch (m_FileSource)
            {
                case FileSourceType.FilePath:
                    tempsplit = m_FilePath.Split('/');
                    tempdot = tempsplit[tempsplit.Length-1].Split('.');
                    if (!tempdot[tempdot.Length-1].Contains("pdf")) return m_FilePath;
                    //for (int t = 0; t < tempdot.Length - 1; t++) tempfilename += tempdot[t];
                    tempfilename = NameChecker(tempdot, tempslash, tempbslash);
                    if (!string.IsNullOrEmpty(tempfilename)) m_FileName = tempfilename;
                    return m_FilePath;
        ...
        private static string NameChecker(string[] dot, string[] slash, string[] bslash)
        {
            if (dot[dot.Length-2].Contains("/")) 
            {
                slash = dot[dot.Length-2].Split('/');
                if (!slash.Contains("\\")) return slash[slash.Length-1];
                bslash = slash[slash.Length-1].Split('\\');
                return bslash[bslash.Length-1];
            }
            if (dot[dot.Length-2].Contains("\\"))
            {
                bslash = dot[dot.Length-2].Split('\\');
                if (!bslash.Contains("/")) return bslash[bslash.Length-1];
                slash = bslash[bslash.Length-1].Split('/');
                return slash[slash.Length-1];
            }
            return dot[dot.Length-2];
        }
        ...
        public void GoToNextPage()
        {
            ...
            PageButtonChecker(mostVisiblePage);
        }
        ...
        private int GetMostVisiblePageIndex()
        {
            ...
            PageButtonChecker(mostVisibleIndex);

            return mostVisibleIndex;
        }

        private void PageButtonChecker(int mostVisibleIndex)
        {
            Image childimg = null;

            m_Internal.PageUpButton.interactable = mostVisibleIndex - 1 >= 0;
            
            foreach (Transform child in m_Internal.PageUpButton.transform)
            {
                if (!child.GetComponent<Image>()) continue;
                childimg = child.GetComponent<Image>();
                Color childimgcolor = childimg.color;
                childimgcolor.a = mostVisibleIndex - 1 < 0 ? 0.5f : 1f;
                childimg.color = childimgcolor;
            }
            
            m_Internal.PageDownButton.interactable = mostVisibleIndex + 1 < m_PageCount;

            foreach (Transform child in m_Internal.PageDownButton.transform)
            {
                if (!child.GetComponent<Image>()) continue;
                childimg = child.GetComponent<Image>();
                Color childimgcolor = childimg.color;
                childimgcolor.a = mostVisibleIndex + 1 >= m_PageCount ? 0.5f : 1;
                childimg.color = childimgcolor;
            }
        }
        ...
        private void SetPageCountLabel(int pageIndex, int pageCount)
        {
                ...
                || m_LastSetLabelPageCount == null || m_LastSetLabelPageCount.Value != pageCount)
			{
                ...
                m_Internal.PageCountLabel.text = " of " + pageCount;
        ...
        {
            #region If PDF Requires a Password
        ...
        public void CheckDocAndContainer()
        {
            bool zoomHasChanged = m_PreviousZoom != 0.0f && Math.Abs(m_PreviousZoom - m_ZoomFactor) > float.Epsilon;
        ...
        public void UpdateScrollBarVisibility()
        {
            ...
            {
                m_Internal.Viewport.offsetMax = new Vector2(-70.0f, m_Internal.Viewport.offsetMax.y);
        ...
        private void ZoomCommon(Vector2 zoomPosition, bool zoomIn, bool useSpecificZoom = false, float specificZoom = 1.0f)
        {
            ...
            else
            {
                ...
                if (zoomIn)
                {
                    ...
                    if (m_ZoomOutMaxed) m_ZoomOutMaxed = !m_ZoomOutMaxed;
                }
                else
                {
                    if (!Mathf.Approximately(Mathf.Floor(m_ZoomToGo * (1 / step)), m_ZoomToGo * (1 / step))
                    ...
                    m_ZoomToGo = Mathf.Clamp(Mathf.Floor((m_ZoomToGo - step) * (1 / step) + epsilon) * step, m_MinZoomFactor, m_MaxZoomFactor);
                    //if (m_ZoomOutMaxed && Math.Abs(m_ZoomToGo - m_MinZoomFactor) <= 0.0f && m_Internal.LeftPanel.IsOpened) m_Internal.LeftPanel.Toggle();
                    //if (Math.Abs(m_ZoomToGo - 0.75f) <= 0.0f) m_ZoomOutMaxed = true;
                }
            }
