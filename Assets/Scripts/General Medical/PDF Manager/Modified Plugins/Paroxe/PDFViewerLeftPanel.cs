...
namespace Paroxe.PdfRenderer.Internal.Viewer
{
    public class PDFViewerLeftPanel : UIBehaviour
    {
        ...
        [SerializeField]
        private TMP_Text m_BookmarksTabTitle;
        [SerializeField]
        ...
        [SerializeField]
        private TMP_Text m_ThumbnailsTabTitle;
        [SerializeField]
        ...
        [Range(0f, 10f)][SerializeField] private float m_DotweenTime = 0.25f;
		...
		public float DotweenTime
        {
	        get => m_DotweenTime;
	        set => m_DotweenTime = value;
        }
        ...
        private void UpdateGraphics()
        {
            ...
            RectTransform.DOScaleX(m_Opened ? 1f : 0f, m_DotweenTime);

            RectTransform.sizeDelta = m_Opened ? new Vector2(m_LastPanelWidth, RectTransform.sizeDelta.y) 
	            : new Vector2(24.0f, RectTransform.sizeDelta.y);

            /*DOTween.To(() => RectTransform.sizeDelta, x => RectTransform.sizeDelta = x, m_Opened ?
		            new Vector2(m_LastPanelWidth, RectTransform.sizeDelta.y) : new Vector2(24.0f, RectTransform.sizeDelta.y),
	            m_DotweenTime);*/

#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN
            if (m_Opened) Cursor.SetCursor(null, Vector2.zero, CursorMode.Auto);
#endif
        }
        ...
        private void UpdateViewport()
        {
	        ...
	        if (!(Math.Abs(viewport.offsetMin.x - RectTransform.sizeDelta.x) > 0.01f)) return;
	        
	        /*DOTween.To(() => viewport.offsetMin, x => viewport.offsetMin = x,
		        new Vector2(RectTransform.sizeDelta.x + 60, viewport.offsetMin.y), m_DotweenTime);*/
	        DOTween.To(() => horizontalScrollBar.offsetMin, x => horizontalScrollBar.offsetMin = x,
		        new Vector2(RectTransform.sizeDelta.x, horizontalScrollBar.offsetMin.y), m_DotweenTime);
